...
class Spanner extends service_1.GrpcService {
    ...
    configureMetrics_() {
        ...
        if (metricsEnabled) {
            const factory = metrics_tracer_factory_1.MetricsTracerFactory.getInstance(this.projectId);
            const periodicReader = new sdk_metrics_1.PeriodicExportingMetricReader({
                exporter: new spanner_metrics_exporter_1.CloudMonitoringMetricsExporter({ auth: this.auth }),
    ...
    request(config, callback) {
        ...
        if (config.client === 'SpannerClient') {
            metricsTracer =
                metrics_tracer_factory_1.MetricsTracerFactory?.getInstance()?.createMetricsTracer(config.method, config.reqOpts.session ?? config.reqOpts.database, config.headers['x-goog-spanner-request-id']) ?? null;
        }
...
const cleanup = async () => {
    if (cleanupCalled)
        return;
    cleanupCalled = true;
    await metrics_tracer_factory_1.MetricsTracerFactory.resetInstance();
};
