 ...
 */
const MetricInterceptor = (options, nextCall) => {
    return new google_gax_1.grpc.InterceptingCall(nextCall(options), {
        start: function (metadata, listener, next) {
            // Record attempt metric on request start
            const factory = metrics_tracer_factory_1.MetricsTracerFactory.getInstance();
            const requestId = metadata.get('x-goog-spanner-request-id')[0];
            ...
            const newListener = {
                onReceiveMetadata: function (metadata, next) {
                    ...
                    if (metricsTracer) {
                        ...
                        const gfeTiming = metricsTracer?.extractGfeLatency(serverTimingHeader);
                        metricsTracer.gfeLatency = gfeTiming ?? null;
                        const afeTiming = metricsTracer?.extractAfeLatency(serverTimingHeader);
