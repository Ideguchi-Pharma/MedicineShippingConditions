...
class Snapshot extends events_1.EventEmitter {
    ...
    _updatePrecommitToken(resp) {
        if (this._latestPreCommitToken === null ||
    ...
    _setMutationKey(mutations) {
        ...
        const [highPriority, lowPriority] = mutations.reduce((acc, mutation) => {
        ...
        if (highPriority.length > 0) {
            ...
            const { bestCandidates } = lowPriority.reduce((acc, mutation) => {
     ...
     */
    _setPreviousTransactionId(transaction) {
        transaction.begin.readWrite = Object.assign({}, transaction.begin.readWrite || {}, {
            multiplexedSessionPreviousTransactionId: this.multiplexedSessionPreviousTransactionId,
        });
    }
    begin(gaxOptionsOrCallback, cb) {
        ...
        if (this.multiplexedSessionPreviousTransactionId &&
            this.session.parent.isMuxEnabledForRW_) {
        ...
        return (0, instrument_1.startTrace)('Snapshot.begin', {
    ...
    createReadStream(table, request = {}) {
        ...
        return (0, instrument_1.startTrace)('Snapshot.createReadStream', traceConfig, span => {
            let attempt = 0;
    ...
    read(table, requestOrCallback, cb) {
        ...
        return (0, instrument_1.startTrace)('Snapshot.read', {
            tableName: table,
            ...this._traceConfig,
        }, span => {
    ...
    run(query, callback) {
        ...
        let stats;
        let metadata;
        (0, instrument_1.startTrace)('Snapshot.run', {
            ...query,
            ...this._traceConfig,
        }, span => {
    ...
    runStream(query) {
        ...
        return (0, instrument_1.startTrace)('Snapshot.runStream', traceConfig, span => {
            let attempt = 0;
            const database = this.session.parent;
            const nthRequest = (0, request_id_header_1.nextNthRequest)(database);
            const makeRequest = (resumeToken) => {
    ...
    static encodeTimestampBounds(options) {
        ...
        if ((0, helper_1.isEmpty)(readOnly)) {
...
class Dml extends Snapshot {
    runUpdate(query, callback) {
        ...
        return (0, instrument_1.startTrace)('Dml.runUpdate', {
            ...query,
            ...this._traceConfig,
            transactionTag: this.requestOptions?.transactionTag,
...
class Transaction extends Dml {
    ...
    batchUpdate(queries, optionsOrCallback, cb) {
        ...
        return (0, instrument_1.startTrace)('Transaction.batchUpdate', traceConfig, span => {
            this.request({
                client: 'SpannerClient',
                ...
                if (err) {
                    ...
                    (0, instrument_1.setSpanError)(span, batchUpdateError);
                    span.end();
    ...
    commit(optionsOrCallback, cb) {
        ...
        return (0, instrument_1.startTrace)('Transaction.commit', {
            transactionTag: this.requestOptions?.transactionTag,
            ...this._traceConfig,
        }, span => {
            if (this.id) {
                ...
                if (this.session.parent.isMuxEnabledForRW_) {
    ...
    rollback(gaxOptionsOrCallback, cb) {
        const gaxOpts = typeof gaxOptionsOrCallback === 'object' ? gaxOptionsOrCallback : {};
        const callback = typeof gaxOptionsOrCallback === 'function' ? gaxOptionsOrCallback : cb;
        return (0, instrument_1.startTrace)('Transaction.rollback', this._traceConfig, span => {
            if (!this.id) {
...
function buildMutation(method, table, keyVals) {
    const rows = (0, helper_1.toArray)(keyVals);
...
function buildDeleteMutation(table, keys) {
    const keySet = {
        keys: (0, helper_1.toArray)(keys).map(codec_1.codec.convertToListValue),
    };
    const mutation = {
        delete: { table, keySet },
    };
...
class PartitionedDml extends Dml {
    ...
    runUpdate(query, callback) {
        return (0, instrument_1.startTrace)('PartitionedDml.runUpdate', {
            ...query,
            ...this._traceConfig,
        }, span => {
